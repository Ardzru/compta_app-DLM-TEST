...
def nettoyer_montant(val):
    ...
    - Gère le format AMEX négatif suffixé (ex: '570,00-')
...
def traiter_amex_caisse(fichier: Path):
    """
    Traite un fichier AMEX Caisse et génère les écritures comptables.

    Règles métier principales :
    - Seules les lignes avec signature 'DLM' sont traitées
    - Trois cas gérés :
        1. Remboursement client
        2. Encaissement simple
        3. Encaissement avec frais AMEX
    """

    # Choix du moteur Excel selon le format du fichier
    ...
    COL_G = 6              # Frais AMEX
    COL_I = 8              # Montant net
    COL_SIGNATURE = 14     # Signature (DLM / SITE)
    ...
    for _, row in df.iterrows():
        ...
        # Filtrage AMEX Caisse uniquement
    ...
    if lignes:
        ...
        sortie = DOSSIER_SORTIE / f"{fichier.name}_amex_caisse.csv"
