...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('justification_debug.log')
    ]
)
logger = logging.getLogger('justification')
...
def _normaliser_cmd(val):
    ...
    try:
        ...
        return s if RE_CMD.match(s) else None
    except Exception as e:
        logger.error(f"Normalisation échouée pour '{val}': {e}")
        return None

def _get_alpi_key(row):
    return _normaliser_cmd(row.get(COL_ALPI_ID_CMD))
...
class JustificationHandler:
    ...
    def _extraire_commandes_banque(self, df):
        ...
        for _, row in df.iterrows():
            try:
                commande = _normaliser_cmd(row.get(COL_BANQUE_COMMAND))
                montant  = row.get(COL_BANQUE_MONTANT)
                if commande and montant:
                    ...
                    commandes.append({
                        ...
                        "montant":       montant,
                        "source":        "banque",
                        "date":          row.get(COL_BANQUE_DATE),
                        "email":         row.get(COL_BANQUE_EMAIL),
                        "type_paiement": row.get(COL_BANQUE_TYPE),
                        "statut":        row.get(COL_BANQUE_STATUT),
                        "contrat":       row.get(COL_BANQUE_CONTRAT),
                        "n_remise":      row.get(COL_BANQUE_REMISE),
                        "date_remise":   row.get(COL_BANQUE_DATE_REMISE),
                    })
            except Exception as e:
    ...
    def _extraire_commandes_alpilink(self, df):
        ...
        for _, row in df.iterrows():
            try:
                ...
                montant  = row.get(COL_ALPI_MONTANT)
                if commande and montant:
                    montant = float(str(montant).replace(' ', '').replace(',', '.'))
                    commandes.append({
                        "commande":   commande,
                        "montant":    montant,
                        "source":     "alpilink",
                        "statut":     row.get(COL_ALPI_STATUT),
                        "canal":      row.get(COL_ALPI_CANAL),
                        "id_portail": row.get(COL_ALPI_ID_PORTAIL),
                    })
            except Exception as e:
    ...
    def _extraire_commandes_compta(self, df):
        ...
        for _, row in df.iterrows():
            try:
                libelle  = row.get(COL_COMPTA_PIECE)
                montant  = row.get(COL_COMPTA_MONTANT)
                date     = row.get(COL_COMPTA_DATE)
                journal  = row.get(COL_COMPTA_JOURNAL)
                ...
                if commande and montant:
                    ...
                    commandes.append({
                        ...
                        "date":     date,
                        "source":   "compta",
                        "libelle":  row.get(COL_COMPTA_LIBELLE),
                        "journal":  journal,
                    })
